...
SELECT IZENA,ABIZENAK
FROM BEZERO
WHERE IZENA LIKE 'B%' AND AKTIBO ='B';

SELECT IZENBURUA
FROM PELIKULA
WHERE ID = (SELECT FROM )

SELECT COUNT(ID_PELIKULA)
FROM KATEGORIA_PELIKULA
WHERE 
...
SELECT MAX(ID_INBENTARIO)
FROM ALOKAIRU;
DELETE FROM INBENTARIO WHERE ID_DENDA = 2 AND ID_INBENTARIO = (SELECT ID_INBENTARIO FROM ALOKAIRU WHERE ID_INBENTARIO NOT BETWEEN 1 AND 4581);
