...
SELECT IZENA,JAIOTZEDATA, TO_CHAR(JAIOTZEDATA,'YYYY') || 'KO ' ||  RTRIM(TO_CHAR(JAIOTZEDATA,'MONTH','NLS_DATE_LANGUAGE = BASQUE')) || ' ' || TO_CHAR(JAIOTZEDATA,'DD') || 'A' AS DATA FROM LANGILEAK;
...
SELECT L.IZENA, S.IZENA AS SAILA, UPPER(LA.EGINKIZUNA) AS EGINKIZUNA FROM LANGILEAK L, SAILAK S, LANEAN LA WHERE L.KODEA = LA.LANGILE_KODEA AND S.KODEA = LA.SAIL_KODEA;

/*Langile guztien izena, jaioteguna eta jaiotegunaren asteko egunaren izena*/

SELECT IZENA, JAIOTZEDATA, TO_CHAR(JAIOTZEDATA, 'DAY') AS EGUNA FROM LANGILEAK;

/*Langileen izena eta adina*/

SELECT IZENA, TRUNC(MONTHS_BETWEEN(SYSDATE,JAIOTZEDATA)/12) AS ADINA FROM LANGILEAK;

/*50 urte baino gutxiago izan eta seme-alabak dituzten langileen izena, adina eta seme-alaba kopurua*/

SELECT IZENA, TRUNC(MONTHS_BETWEEN(SYSDATE,JAIOTZEDATA)/12) AS ADINA, SEMEALABAK FROM LANGILEAK WHERE TRUNC(MONTHS_BETWEEN(SYSDATE,JAIOTZEDATA)/12) < 50 AND SEMEALABAK > 0;
...
SELECT L.IZENA, SUM(N.DIRUSARRERA) AS DIRUA
