...
SELECT MAX(SALMENTAPREZIOA) AS MAXIMOA, MIN(SALMENTAPREZIOA) AS MINIMOA
FROM PRODUKTUAK;
...
SELECT COUNT(BEZEROKODEA) AS HERRIAK
FROM BEZEROAK
WHERE HERRIA LIKE 'M%';

-- "M" hizkitik hasten diren herri bakoitzean dagoen bezero kopurua.

SELECT HERRIA, COUNT(BEZEROKODEA) AS BEZEROAK
FROM BEZEROAK
WHERE HERRIA LIKE 'M%'
GROUP BY HERRIA;
...
-- Ordainketarik egin ez duten bezeroen izena, beraien salerosketa ordezkariaren izenarekin batera.
