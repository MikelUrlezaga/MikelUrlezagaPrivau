 ...
 * Kontsulta, Eskarien informazioa ikusteko balio du
 * @author Talde_2
 * @see DBConexion
 */
public class Eskariak extends DBConexion {
    ...
    protected JLabel aux2 = new JLabel("Eskariaren id: ");
     ...
     * @see #crearTabla()
     */
    public Eskariak(String direccion, String erab , String pass) {
        crearTabla();
        this.url = direccion;
    ...
    /**
     * Eskariak ikusteko pantaila sortzeko
     * @see #posicionar()
     * @see #cososDeBoton()
     * @see #kllogin()
     * @see #jcb()
    ...
    protected void bGui() {
        ...
        ventana.setTitle("Eskariak");
        ventana.setLocationRelativeTo(null);
        ventana.setVisible(true);
    }
    /**
     * Eskarien egoerak JCB batean zartzeko
     * @see #connect()
     * @see #consulta()
     * @see #cerrar()
     */
    protected void jcb() {
        connect();
        this.select = "ID, DESKRIBAPENA";
        this.from = "ESKARI_EGOERA";
        this.where = "";
        consulta();
        try {
            while (this.rs.next()) {
                String egoeras = rs.getString("ID") + " - " + rs.getString("DESKRIBAPENA");
                jcb.addItem(egoeras);
            }
            cerrar();
        } catch (Exception e) {
        ...
        for (int i = 0; i < filas; i++) {
            ...
            if (auxid.equals(aux.getText())) {
                fila = modelo.getColumnName(0) + ": " + modelo.getValueAt(i, 0) + " - " + modelo.getColumnName(1) + ": " + modelo.getValueAt(i, 1) + " - "
                        ...
                        modelo.getColumnName(3) + ": " + modelo.getValueAt(i, 3) + " - " + modelo.getColumnName(4) + ": " + modelo.getValueAt(i, 4) + " - ";
                JOptionPane.showMessageDialog(null, fila, auxid + "Eskariak", JOptionPane.INFORMATION_MESSAGE);
            }
        }
    }
    ...
    /**
     * Eskarien informazioa izateko funtzioa
     * @see #connect()
     * @see #cerrar()
     * @see #consulta()
     */
    protected void xuclarRS() {
        connect();
        this.select = "E.ID, EG.DESKRIBAPENA, E.ESKAERA_DATA, (B.IZENA || ' ' || B.ABIZENA) AS BEZEROA, (L.IZENA || ' ' || L.ABIZENA) AS SALTZAILEA";
        this.from = "ESKARI E, ESKARI_EGOERA EG, BEZERO B, LANGILE L";
        this.where = "E.ID_EGOERA=EG.ID AND E.ID_BEZERO=B.ID AND E.ID_SALTZAILE=L.ID";
        consulta();
        try {
            while (this.rs.next()) {
                tinfo[0] = rs.getString("ID");
                tinfo[1] = rs.getString("DESKRIBAPENA");
                tinfo[2] = rs.getString("ESKAERA_DATA");
                tinfo[3] = rs.getString("BEZEROA");
                tinfo[4] = rs.getString("SALTZAILEA");
    ...
    /**
     * Eskarien egoera zeatz bateko informazioa izateko funtzioa
     * @see #connect()
     * @see #cerrar()
     * @see #consulta()
     */
    protected void xuclarRS2() {
        ...
        this.select = "E.ID, EG.DESKRIBAPENA, E.ESKAERA_DATA, (B.IZENA || ' ' || B.ABIZENA) AS BEZEROA, (L.IZENA || ' ' || L.ABIZENA) AS SALTZAILEA";
        this.from = "ESKARI E, ESKARI_EGOERA EG, BEZERO B, LANGILE L";
        this.where = "E.ID_EGOERA=EG.ID AND EG.ID =" + id + " AND E.ID_BEZERO=B.ID AND E.ID_SALTZAILE=L.ID";
        consulta();
        try {
            while (this.rs.next()) {
                tinfo[0] = rs.getString("ID");
    ...
    protected void crearTabla() {
        modelo = new DefaultTableModel(null,
                new String[] { "ID", "DESKRIBAPENA", "ESKAERA_DATA", "BEZEROA", "SALTZAILEA", }) {
